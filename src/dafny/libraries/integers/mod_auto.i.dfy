...
lemma lemma_mod_yourself_negated(n:int)
requires  n >0
ensures -n % n == 0
{}
...
lemma lemma_mod_auto_induction(n:int, x:int, f:imap<int,bool>)
    ...
    requires ModAuto(n) ==> (forall i :: TModAutoLe(0, i) && i < n ==> f[i])
                        && (forall i :: TModAutoLe(0, i) && f[i] ==> f[i + n])
                        && (forall i :: TModAutoLe(i + 1, n) && f[i] ==> f[i - n]);
    ensures  ModAuto(n);
    ensures  f[x];
{
    lemma_mod_auto(n);
